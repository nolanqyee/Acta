...
| `brand-design-system.md` | Full visual system (tokens, type, colour, "liquid glass" surfaces, motion) locked before anything was rendered. The glass direction in particular produced unreadable translucent modals over live content. Tokens still live in `src/styles/tokens.css`; the visual system will be rebuilt from working screens rather than specified up front. |
...
[`../agent-interaction-model.md`](../agent-interaction-model.md). The idea and the
