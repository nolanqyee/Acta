...
- **Verification:** Dogfood on `/` and `/lab/graph`: hover, selection, left detail panel verified on screen (2026-07-28). Graph-home chrome overlays (ask, capture, Explore, …) **not** on `/` — mocked at `/lab/design` only.
- **Status:** ✅ **U3 canvas engine + interaction slice shipped** (rebuilt 2026-07-26; interaction polish verified 2026-07-28). See [`graph-canvas.md`](graph-canvas.md) for the live spec and module map.
  ...
  **What landed:** `simulation.ts`, `camera.ts`, `render.ts`, `seed.ts`, `palette.ts`, `graph-canvas.tsx` (RAF loop, our own `d3-force` tick), `graph-view.tsx` (fetch + hover/selection/detail on `/`), `node-detail.tsx`, `hover-card.tsx`, `dev-hud.tsx` + `/lab/graph` workbench. Verification: look at `/lab/graph` and `/`; `src/test/graph/layout-shape.test.ts` guards the real engine (roundness, locality, shimmer).

  **Parallel (U-A, not U3 exit):** Neubrutalism tokens, `/lab/design` chrome mock, `/landing` waitlist — see [`design-handoff.md`](design-handoff.md). **Light mode only** (dark deferred).

  **Still deferred to later units:** ask bar, filter, graph settings, capture, deepen, Explore/diff-skim panels, empty-state CTA, NL ranking, real deepen signal — each returns as its own reviewed slice (U4–U6).

  **Removed with first attempt:** `react-force-graph-2d`, centered modal + scrim, full chrome built in one pass, Playwright screenshot scripts (`shoot-graph.mjs` — references remain in comments; script file gone).
...
- **2026-08-03:** **U3 status block rewritten** to match current code: canvas + hover/selection/detail shipped; chrome overlays deferred to lab/U4–U6; light mode only; removed stale theme-module and full-chrome references. Added [`docs/file-catalogue.md`](file-catalogue.md) as repo inventory.
